...
//add new
#define Shift_ctr_pos 1
#define Ctrl_ctr_pos 0

uint8_t KeyboardReportParser::OemToAscii4Barcode(uint8_t mod, uint8_t key) {
        
        // mod &=~(1<<Shift_ctr_pos); //disable shift
        // mod &=~(1<<Ctrl_ctr_pos); //disable control

        uint8_t shift = (mod & 0x22);
        //true with barcode x-9200L
